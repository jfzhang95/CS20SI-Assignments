...
def _gram_matrix(F, N, M):
    ...
    return tf.matmul(tf.transpose(F_reshaped), F_reshaped)
...
def _single_style_loss(a, g):
    ...
    return tf.reduce_sum((G - A) ** 2) / ((2 * M * N) ** 2)
    ...
    with tf.Session() as sess:
        ...
        for index in range(initial_step, ITERS):
            ...
            if (index + 1) % skip_step == 0:
                ...
                gen_image, total_loss, summary = tf.run([generated_image, model['total_loss'], model['summary_op']])
                ###############################
